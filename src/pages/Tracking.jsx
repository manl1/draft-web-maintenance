...
export default function Tracking() {
  ...
  // Auto-expand row setelah data ready
  useEffect(() => {
    const state = location.state
    if (!state?.autoExpand || !isReady || rows.length === 0) return
    const targetId = state.assetId
    if (!targetId) return
    // Cek apakah asset tersebut ada di rows dan has_p2h
    const found = rows.find(r => String(r.asset_id) === String(targetId) && r.has_p2h)
    if (found) {
      setExpandedRows(prev => ({ ...prev, [targetId]: true }))
